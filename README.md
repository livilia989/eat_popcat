...
| `popcat_pop.mp3` | 팝캣이 입을 벌릴 때마다 — 한 입에 **두 번** "뻐끔뻐끔" |
| `snack_eat.mp3` | 먹기 완료 |
| `mood_up.mp3` | 기분 상승 |
...
`oiia_loop.mp3` (파티 BGM)는 원본 곡에서 가장 신나는 구간을 잘라 쓴다.
템포를 추정해 다운비트에 맞추고 마디 단위로 자르므로 어중간하게 끊기지 않는다.

```bash
python3 tools/make_party_bgm.py <원본곡.mp3> [시작초] [마디수]
```

시작초·마디수를 생략하면 에너지가 높고 기복이 적은 8초 구간을 자동으로 고른다.
현재 파일은 148 BPM 기준 5마디(8.11초)다 — 파티(8초)보다 살짝 길어
이벤트 도중에 곡이 떨어지지 않는다.

효과음은 코드로 합성한 것이라 원하는 소리로 교체해도 된다
(`python3 tools/make_sounds.py [이름]` 으로 하나만 다시 만들 수도 있다).
...
__tests__/                   자동 테스트 64개
```

### 에셋 소스는 플랫폼마다 형태가 다르다

`require('...')` 의 반환값이 다르므로 한 가지 형태만 받아들이면 안 된다.

| 플랫폼 | 반환 형태 |
|---|---|
| 네이티브(Android/iOS) | 숫자 (에셋 레지스트리 id) |
| 웹 | 문자열(URL / data URI) 또는 `{ uri }` 객체 |

`constants/assets.ts` 의 `resolveAssetSource()` 가 셋 다 통과시키고 빈 값만
`null` 로 떨어뜨린다. 예전에는 숫자와 객체만 받아서 **웹에서 사운드가 전부
`null` 이 되어 소리가 하나도 나지 않았다.** 무음 처리가 조용히 일어나
알아채기도 어려웠기 때문에 `__tests__/assets.test.ts` 로 고정해 두었다.
...
64개 테스트 / 7개 스위트, 요구사항의 테스트 시나리오를 코드로 옮긴 것이다.
...
| `assets.test.ts` | 에셋 소스 해석 (숫자/문자열/객체), 무음 처리된 사운드 없음 |
